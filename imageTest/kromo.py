...
if __name__ == '__main__':
    ...
    # im = im.resize((im.size[0]-1, im.size[1]-1))
    # im = add_chromatic(im)
    # im.save("test2.png")
#     import argparse
...
#     ifile = args.filename
    ...
    og_im = im.copy()
    _start = time.time()
    im = add_chromatic(im, no_blur=False)
    _stop = time.time()
    print(f"time taken by function: {(_stop-_start)*1000} milliseconds")
    # Add Jitter Effect
    # im = add_jitter(im, pixels=1)

    # im = blend_images(im, og_im, alpha=0.0)
